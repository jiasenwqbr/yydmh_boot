    ...
    public LoginUser checkUserTokenIsEffect(String token) throws AuthenticationException {
       ...
       log.debug("———校验token是否有效————checkUserTokenIsEffect——————— "+ token);
        ...
        if (loginUser == null) {
            ...
            String sql = "select * from mh_user_register_info where id = :id and del_flag = 0";
            List<Map<String, Object>> users = namedParameterJdbcTemplate.queryForList(sql,param);
            if(users.size() > 0){
                 Map<String, Object> user =  users.get(0);
                 if(user.get("status").equals(2)){
