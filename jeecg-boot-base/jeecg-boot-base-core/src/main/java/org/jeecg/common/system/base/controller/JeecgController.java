...
public class JeecgController<T, S extends IService<T>> {
    ...
    protected ModelAndView exportXls(HttpServletRequest request, T object, Class<T> clazz, String title) {
        ...
        if (oConvertUtils.isNotEmpty(selections)) {
            ...
            System.out.println("...................................总数量："+pageList.size());
            ...
            if (exportList.size()>10000){
                System.out.println("...................................导出数据超长");
                pageList = pageList.subList(1,10000);
            }
        }
    ...
    protected ModelAndView exportXlsTemplate(HttpServletRequest request, T object, Class<T> clazz, String title) {
        // Step.1 组装查询条件
        QueryWrapper<T> queryWrapper = QueryGenerator.initQueryWrapper(object, request.getParameterMap());
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        // Step.2 获取导出数据
...
//
//        }
        //没有ID字段会报错 queryWrapper.eq("id","1111111111111");
        queryWrapper.eq("1","1111111111111");
        List<T> pageList = service.list(queryWrapper);
        exportList = pageList;
        // Step.3 AutoPoi 导出Excel
        ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
    ...
    protected ModelAndView exportXlsError(HttpServletRequest request, T object, Class<T> clazz, String title) {
        // Step.1 组装查询条件
        QueryWrapper<T> queryWrapper = QueryGenerator.initQueryWrapper(object, request.getParameterMap());
        queryWrapper.eq("gy_bj","2");
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        // Step.2 获取导出数据
        System.out.println("开始导出检索：");
        List<T> exportList = null;
    ...
    protected Result<?> importExcelError(HttpServletRequest request, HttpServletResponse response, Class<T> clazz) {
        MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
        Map<String, MultipartFile> fileMap = multipartRequest.getFileMap();
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            MultipartFile file = entity.getValue();// 获取上传文件对象
